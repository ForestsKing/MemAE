...
from model.entropyloss import EntropyLossEncap


def train(X_train, scaler, model, lr=0.005, epochs=100, batch_size=16, patience=10, Q=0.99, validation_split=0.2,
          nstep=64, entropy_loss_weight=0.0001, path='log/', showloss=True):
    X_test = X_train
    ...
    for epoch in range(epochs):
        ...
        for i, (batch_x, batch_y) in enumerate(dataloader_train):
            ...
            recon_loss = recon_criterion(output, batch_y)
            entropy_loss = entropy_criterion(attention)
            loss = recon_loss + entropy_loss_weight * entropy_loss
